...
任务：
1. 在创建旅行时，上传了封面图片，然后保存旅行时提示创建旅行失败: Internal server error，请修正
2. 现在旅行界面不显示数据库中的任何旅行了，是因为你在Trip模型中添加了一个字段cover_image_url吗？你可以把现有数据库中的所有旅行的cover_image_url字段都设为空字符串，然后重新运行项目，看看是否能够正常显示数据库中的所有旅行。
3. 在社区和帖子详情页面根本没有看到该帖子关联的旅行（现在暂时不是点击后是否跳转的问题，是根本看不到关联的旅行），请修复
