...
1. 现在admin的交通管理页面和活动管理页面仍然出现错误
2. 现在在发布帖子时可以上传多张图片，这多张图片也都可以存到post_images表中，但是在帖子详情页面中只会显示display_order为0的照片。我希望你添加左右翻动的按钮，支持查看所有图片





客观地依照软件工程的最佳实践和领域驱动设计的原则回答我，不要附和我！！
为了达成以下要求，这两个问题的答案是什么？
问题：
1. 好友的逻辑写在 app_social中，那么需要在app_social的领域层增加什么值对象、实体、聚合根、领域服务、领域事件？基础设施层需要增加什么？
2. 增加了好友的逻辑后，app_social的应用层要如何编排领域层（聚合根、领域服务）的服务，才能实现这些好友功能
3. app_social的接口层view要暴露给前端哪些端口？
