...
const TripMap = ({ activities = [], transits = [], initialCenter }) => {
    ...
    // 检测测试环境
    useEffect(() => {
        const isTest = typeof window !== 'undefined' && window.__TEST_MODE__ === true;
    ...
    useEffect(() => {
        if (isTestMode) {
            const fakeCenter = center || [116.397428, 39.90923];
            window.__TEST_MAP_INSTANCE__ = {
                getCenter: () => ({
                    getLng: () => fakeCenter[0],
                    getLat: () => fakeCenter[1],
                }),
            };
            window.__TEST_MARKERS__ = markers.map((marker) => ({
                ...marker,
                position: {
                    getLng: () => marker.position[0],
                    getLat: () => marker.position[1],
                },
            }));
    ...
    }, [center, isTestMode, map, markers, polylines]);

    const handleMapInstance = (instance) => {
        const nextMap = instance?.map ?? instance;
        if (nextMap && nextMap !== map) {
            console.log('Map Instance acquired');
            setMap(nextMap);
        }
    };
        ...
        <div data-testid="trip-map-container" style={{ width: '100%', height: '600px', borderRadius: '0.5rem', overflow: 'hidden', border: '1px solid #e2e8f0', position: 'relative' }}>
            <APILoader akey={import.meta.env.VITE_AMAP_KEY}>
                <Map
                    ...
                    center={center}
                    viewMode="2D"
                    onInstanceCreated={handleMapInstance}
