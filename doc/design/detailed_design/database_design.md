...
### 1.2 设计原则
1.  **标准化 (Normalization)**：原则上遵循第三范式 (3NF)，以减少数据冗余和保证数据一致性。在特定场景（如 信息流查询）下，允许适当的反范式化设计（如 JSON 字段存储）以优化读取性能。
...
### 4.3 性能优化建议
1.  **信息流查询优化**：`posts` 表查询时，应始终带上 `is_deleted=0`。对于首页广场流，建议使用 `created_at` 建立复合索引，或者引入 Redis 进行 ID 列表的缓存。
