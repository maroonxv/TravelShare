...
*   **选型理由**：DeepSeek 提供了高性能的大语言模型能力。结合 RAG 技术，利用本地数据库（Activity, Trip, Post）作为知识库，解决了通用大模型在特定旅行场景下"幻觉"严重的问题，提供了精准的个性化建议。
...
### 6.2 事务管理
事务边界通常划定在**应用服务层**的方法级别。一个 Service 方法对应一个数据库事务。我们使用 Flask 的 `g` 对象或装饰器来管理 Session 的生命周期，确保请求结束时自动提交或回滚。
