...
partition "服务器 (工作线程)" {
    "发送 HTTP 请求" --> "获取 DB 连接 (连接池检出)"
    "获取 DB 连接 (连接池检出)" --> "开启事务 (Transaction Begin)"
...
@startuml
actor "用户" as User
participant "浏览器 (React)" as Client
participant "Flask 工作线程" as Server
participant "LLM 客户端" as Adapter
...
```plantuml
@startuml
participant "管理员/脚本" as User
participant "app.py" as App
participant "配置" as Conf
participant "数据库" as DB
participant "SocketIO" as Socket
participant "蓝图" as BP
